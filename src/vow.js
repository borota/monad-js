...
// In some mythologies, 'VOW' is called 'deferrer'.
  ...
  return {
    ...
    // promise.
      ...
      const breakers = []; // .when's broken queue
      let fate; // The promise's ultimate value
      const keepers = []; // .when's kept queue
      let status = 'pending'; // 'broken'; 'kept'; or 'pending'
      ...
      return {
        ...
        keep: function keepPromise(value) {
        ...
        promise: {
          ...
          when: function makeVow(kept, broken) {
    ...
    kept: function keptPromise(value) {
    ...
    broken: function brokenPromise(reason) {
